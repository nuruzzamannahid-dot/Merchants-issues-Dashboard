...
## 2026-07-31 - [Keyboard Navigation Focus Indicators & Custom Control Accessibility]
**Learning:** Custom interactive elements (e.g. `div`s used as dropdown toggles like `#userAvatar`) lack inherent keyboard accessibility, role definitions, and focus indicators. This makes them entirely invisible to screen readers and keyboard navigation.
**Action:** Always add `role="button"`, `tabindex="0"`, `aria-label`, and `aria-haspopup` to custom interactive div components. Implement `keydown` listeners to handle space/enter selection, and add high-contrast `:focus-visible` styling to custom controls so keyboard-only users can navigate clearly.
